...
if(boundary===undefined||options.autoWrap===true){goToCell=getCellByCoords(goToCol,goToRow);goToCellData=$.data(goToCell,pluginName);eventData={fromIndex:currentCellData.index,toIndex:goToCellData?goToCellData.index:null};$(goToCell).trigger("gridNavigationChange",eventData);currentCol=goToCol;currentRow=goToRow;}};var updateModelByCol=function(colIndex){updateModelByCoords(colIndex,currentRow);};var updateModelByRow=function(rowIndex){updateModelByCoords(currentCol,rowIndex);};var updateModelByIndex=function(index){var $cell=$($allCells.get(index));numCells=$allCells.length;updateModelByCoords($cell.index(),$cell.parent().index());};var onGridNavigationItemsChange=function(){$allCells=$widget.find(cellsSelector);numCells=$allCells.length;$rows=$allCells.first().parent().parent().children();$cols=$rows.first().children();numCols=$cols.length;numRows=$rows.length;storeData();};var onArrowKeyDown=function(e){if(needsInit()===true){initModel();}else{switch(e.type){case"upArrowKeyDown":updateModelByRow(currentRow-1);break;case"downArrowKeyDown":updateModelByRow(currentRow+1);break;case"leftArrowKeyDown":updateModelByCol(currentCol-1);break;case"rightArrowKeyDown":updateModelByCol(currentCol+1);break;default:break;}}};var onHomeKey=function(e){updateModelByCol(0);};var onEndKey=function(e){updateModelByCol(numCols-1);};var onPageUpKey=function(e){updateModelByRow(0);};var onPageDownKey=function(e){updateModelByRow(numRows-1);};$widget.commonKeyDown();$widget.on('upArrowKeyDown downArrowKeyDown rightArrowKeyDown leftArrowKeyDown',onArrowKeyDown);if(options.disableHomeAndEndKeys===false){$widget.on('homeKeyDown',onHomeKey);$widget.on('endKeyDown',onEndKey);}
if(options.disablePageUpAndDownKeys===false){$widget.on('pageDownKeyDown',onPageDownKey);$widget.on('pageUpKeyDown',onPageUpKey);}
$widget.on('click',cellsSelector,function(e){var $this=$(this);if(needsInit()===true){initModel();}
